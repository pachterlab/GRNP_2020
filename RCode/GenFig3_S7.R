...
#                  sampling=ldata$sampling - ldata$nopred)
plotdata = tibble(gene=ldata$gene, 
                  x=ldata$x, 
                  nopred=ldata$nopred - ldata$trueval,
                  pred=ldata$pred - ldata$trueval,
...
plot(plotdata$x, plotdata$poolpred)
plot(plotdata$x, plotdata$pred)
#melt
#plotdata.m = reshape2::melt(plotdata, id.vars=c("gene","x"), measure.vars = c("nopred", "pred", "poolpred", "sampling"))
plotdata.m = reshape2::melt(plotdata, id.vars=c("gene","x"), measure.vars = c("nopred", "pred", "poolpred"))

#labl = labeller(variable = 
#                      c("nopred" = "No Prediction",
#                        "pred" = "Prediction",
#                        "poolpred" = "Prediction with pooling",
#                        "sampling" = "Sampling noise"))
#labl = labeller(variable = 
#                      c("nopred" = "No Prediction",
#                        "pred" = "Prediction",
#                        "poolpred" = "Prediction with pooling"))
...
figS7 # for some reason this plot sometimes fail and show an error ("hbin" ...) - Restart R and try again in that case


ggsave(
  paste0(figure_path, "FigS7.png"),
  plot = figS7, device = "png",
...
#print(paste0("R-Squared from identity, no pred: ", getRSquaredFromIdentity(ldata$nopred, ldata$trueval)))
#print(paste0("R-Squared from identity, pred no pooling: ", getRSquaredFromIdentity(ldata$pred, ldata$trueval)))
#print(paste0("R-Squared from identity, pred with pooling: ", getRSquaredFromIdentity(ldata$poolpred, ldata$trueval)))
#print(paste0("R-Squared from sampling, no pred, ds 100 times vs ds: ", getRSquaredFromIdentity(ldata$nopred, ldata$sampling)))


print(paste0("CCC, no pred: ", getCCC(ldata$nopred, ldata$trueval))) #0.981389618309966
print(paste0("CCC, pred no pooling: ", getCCC(ldata$pred, ldata$trueval))) #0.99392218247023
print(paste0("CCC, pred with pooling: ", getCCC(ldata$poolpred, ldata$trueval))) #0.997066230750738
print(paste0("CCC, no pred, ds 100 times vs ds: ", getCCC(ldata2$nopred, ldata2$sampling))) #0.998928835858109
